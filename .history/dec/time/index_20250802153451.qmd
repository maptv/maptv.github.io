...
& + \beta_1 \times \text{cos}(\gamma) \\
& + \beta_2 \times \text{sin}(\gamma) \\
& + \beta_3 \times \text{cos}(2\gamma) \\
& + \beta_4 \times \text{sin}(2\gamma)
\end{split}$$
...
& + \beta_3 \times \text{cos}(2\gamma) \\
& + \beta_4 \times \text{sin}(2\gamma) \\
& + \beta_5 \times \text{cos}(3\gamma) \\
& + \beta_6 \times \text{sin}(3\gamma) \\
\end{split}$$

## Solar hour angle {.hiddenheading #sha}

With [sda]{.tool data-bs-toggle="tooltip" data-bs-title="solar declination angle"}($\gamma$), we can calculate the "solar hour angle" ([sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"}). In the [sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"} equation below, the "solar zenith angle" ([sza]{.tool data-bs-toggle="tooltip" data-bs-title="solar zenith angle"}) is set to 1.585 radians. The trigonometric functions of most --- if not all -- programming languages accept radians. In general, computer programmers and mathematicians seem to prefer radians over degrees. Instead of radians or degrees, Dec measures angles in turns. To convert turns into radians, we multiply by $\tau$.
...
$$\text{sha} = \pm \arccos\left( \frac{\cos(1.585)}{\cos(\text{lat}) \cdot \cos(\text{decl})} - \tan(\text{lat}) \cdot \tan(\text{decl}) \right) $$

$$\text{minuend} = \text{subtrahend} + \text{diff}$$

  are better because the Dec base units of time, longitude, and latitude can be multiplied by $\tau$ to can convert days, parallels, and meridians into radians simply by multiplying to . Apart from such as solar noon,
