...
At sunrise, the [da]{.tool data-bs-toggle="tooltip" data-bs-title="day arc"} is the amount of time until sunset. At solar noon, the time remaining before sunset will be [da/2]{.tool data-bs-toggle="tooltip" data-bs-title="half day arc"}. Throughout the day, the current tod is equal to the sum of sunset tod and the difference between the current and sunset tods: ${selectedSunset.toFixed(3)} -- ${selectedSunrise.toFixed(3)} = \${parseFloat(daytimeDuration.toFixed(3))}.


Like a countdown sequence, we can keep track of the time relative to sunset throughout the day. Instead of the current tod, we can display the , the  and the current tod is equal to the difference between the sunset tod and the [da]{.tool data-bs-toggle="tooltip" data-bs-title="day arc"} and the current tod is equal to the difference between the sunset tod and the [da/2]{.tool data-bs-toggle="tooltip" data-bs-title="half day arc"}. The Given a sunset tod of We can replace the current tod with an expression that provides the time relative to a given tod.
