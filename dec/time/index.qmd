...
In other words, the difference will be negative if the minuend is less than the substrahend or positive if the minuend is greater than the subtrahend. In this way, the righthand side of a Dec span acts like a "[T minus countdown](https://en.wikipedia.org/wiki/Countdown#:~:text=a%20sequence%20of%20backward%20counting%20to%20indicate%20the%20time%20remaining)" that turns into a ["T plus countup"](https://en.wikipedia.org/wiki/Countdown#:~:text=After%20a%20launch%2C%20most%20countdown%20clocks%20begin%20to%20show%20Mission%20Elapsed%20Time) of [Mission Elapsed Time](https://en.wikipedia.org/wiki/Mission_Elapsed_Time) after the rocket launch. original [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} is called the minuend and the replacement is called the subtrahend. By selecting the current [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} as the minuend and the sunset [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} as the subtrahend, we can keep track of the time before sunset.

$$\text{solartime} \approx \begin{cases}\text{saa}&{\text{if } \phi \geq 0;}\\(1.5 - \text{saa}) \text{ mod } 1&{\text{otherwise.}}\end{cases}$$

The [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} of any event can be the subtrahend in a Dec span. Let’s say that you live in Zone [0]{.color0}, start work at [375]{.color375} md, take a lunch break from [525]{.color525} to [575]{.color575} md, and finish work at [725]{.color725} md. You can have four subtrahends that represent your typical daily rhythm. After one subtrahend passes, you can move on to the next one and thus cycle through all four subtrahends over the course of each day: current = [375]{.color375} diff = [525]{.color525} diff = [575]{.color575} diff = [725]{.color725} diff.

Another common use case for minuend expansion is to replace the current [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} with the timestamp of an event and the time elapsed since that event. To make it easier to compare timestamps, we can agree to always set the [tzo]{.tool data-bs-toggle="tooltip" data-bs-title="time zone offset"} to zero. If we use a non-zero [tzo]{.tool data-bs-toggle="tooltip" data-bs-title="time zone offset"}, we should include it in between the event time and the elapsed time. The time since this webpage was loaded is current - load = diff.
