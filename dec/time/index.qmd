...
Plot.plot({
  ...
  marginLeft: 75,
  y: { label: null },
  x: { grid: true, label: "Population (billions)", labelOffset: 38, transform: d => d},
  marks: [
    Plot.barX(sortedPopByHemi, {x: "Population", y: "Hemisphere", fill: "Offset", stroke: "black", tip: true }),
  ]
})
```

If we only want to know how long the Sun will shine on a given day, we can use the top equation below to obtain a [fda]{.tool data-bs-toggle="tooltip" data-bs-title="full day arc"}. Alternatively, if we are interested in finding out when the Sun will rise or set on a given day, we will need to calculate a [hda]{.tool data-bs-toggle="tooltip" data-bs-title="half day arc"} using the bottom equation below and combine it with a solar noon [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} to get a sunrise or sunset [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"}.

:::{.overflowequation style="overflow-x:auto;overflow-y:hidden;"}
$$\text{fda} = \frac{\arccos\left({\Large\frac{\text{costau(0.252314)} - \text{sintau(sda)} \times \text{sintau($\phi$)}}{\text{costau(sda)} \times \text{costau($\phi$)}}}\right)}{\pi}$$
:::

:::{.overflowequation style="overflow-x:auto;overflow-y:hidden;"}
$$\text{hda} = \frac{\arccos\left({\Large\frac{\text{costau(0.252314)} - \text{sintau(sda)} \times \text{sintau($\phi$)}}{\text{costau(sda)} \times \text{costau($\phi$)}}}\right)}{\tau}$$
:::

The fda and hda equations each require a "solar declination angle" ([sda]{.tool data-bs-toggle="tooltip" data-bs-title="solar declination angle"}) and a latitude. For simplicity, our [sda]{.tool data-bs-toggle="tooltip" data-bs-title="solar declination angle"} and [eot]{.tool data-bs-toggle="tooltip" data-bs-title="equation of time"} functions can differ only in their coefficients, even though the NOAA General Solar Position Calculations propose the more complex sda model shown below. Dec measures latitude in turns called meridians ([&phi;]{.tool data-bs-toggle="tooltip" data-bs-title="meridians"}). Like [&lambda;]{.tool data-bs-toggle="tooltip" data-bs-title="parallels"}, we can pass [&phi;]{.tool data-bs-toggle="tooltip" data-bs-title="meridians"} directly into trigonometric functions that accept turns.

Use the doy, mφ, and latitude range inputs below to find a fda. Sunrise and sunset tods will depend on all three of the range inputs below. We can report the results of our calculations in the form of an equation 

The difference between a sunrise [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} and a sunset [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} is a day arc: ${parseFloat(selectedSunset.toFixed(3))} - ${parseFloat(selectedSunrise.toFixed(3))} = \${parseFloat(daytimeDuration.toFixed(3))}.
...
we need both its solar noon and . We use the latitude of Cambridge, Massachusetts can subtract a "solar hour angle" ([sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"}) from a solar noon [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} to find a sunrise [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"}.
 
## Solar declination angle {.hiddenheading #sda}

The duration of [daytime](https://en.wikipedia.org/wiki/Daytime#:~:text=the%20period%20of%20the%20day%20during%20which%20a%20given%20location%20experiences%20natural%20illumination%20from%20direct%20sunlight) is the difference between the sunset and sunrise [tods]{.tool data-bs-toggle="tooltip" data-bs-title="times of day"}. To calculate the daytime duration, we divide a "solar hour angle" ([sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"}) measured in radians by $\pi$ or subtract a sunrise [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} from a sunset [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"}.
...
\end{split}$$
 ...
 a latitude and a "solar declination angle" ([sda]{.tool data-bs-toggle="tooltip" data-bs-title="solar declination angle"}). For simplicity, we can use the same model for the [eot]{.tool data-bs-toggle="tooltip" data-bs-title="equation of time"} and [sda]{.tool data-bs-toggle="tooltip" data-bs-title="solar declination angle"}, even though the [NOAA]{.tool data-bs-toggle="tooltip" data-bs-title="National Oceanic and Atmospheric Administration"} [General Solar Position Calculations](https://gml.noaa.gov/grad/solcalc/solareqns.PDF) propose the more complex model shown below.

Dec measures latitude in turns called meridians ([&phi;]{.tool data-bs-toggle="tooltip" data-bs-title="meridians"}). Like [&lambda;]{.tool data-bs-toggle="tooltip" data-bs-title="parallels"}, we can pass [&phi;]{.tool data-bs-toggle="tooltip" data-bs-title="meridians"} directly into trigonometric functions that accept turns. The equation below accepts turns but its [arcosine](https://en.wikipedia.org/wiki/Inverse_trigonometric_functions#:~:text=y%20%E2%89%A4%2090%C2%B0-,arccosine,-y%20%3D%20arccos) function provides a "solar hour angle" ([sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"}) in radians. We can then use the resulting [sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"} to find out how long the Sun will shine or when it will rise or set on a given [doy]{.tool data-bs-toggle="tooltip" data-bs-title="day of year"}.

To obtain the duration of daytime or the [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} of sunrise or sunset, the "solar zenith angle" ([sza]{.tool data-bs-toggle="tooltip" data-bs-title="solar zenith angle"}) must be set to 0.252314 turns, but we can use the [doy]{.tool data-bs-toggle="tooltip" data-bs-title="day of year"} and [m&phi;]{.tool data-bs-toggle="tooltip" data-bs-title="millimeridians"} latitude range inputs below to control the other two angles required for the [sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"} equation. The [m&lambda;]{.tool data-bs-toggle="tooltip" data-bs-title="milliparallels"} longitude range input below affects the sunrise and sunset [tods]{.tool data-bs-toggle="tooltip" data-bs-title="times of day"}, but not the daytime duration.

After plugging the three required angles into the equation above, we can divide the resulting [sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"} by $\pi$ to get the daytime duration. Alternatively, we can obtain the daytime duration by subtracting the sunrise [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} from the sunset [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"}. The sunrise [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} is the difference between we first divide the [sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"} by $\tau$ instead of $\pi$. Then, we sum the dividend with the solar noon [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} to get the sunset [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} or subtract it from the solar noon [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"} to obtain the sunrise [tod]{.tool data-bs-toggle="tooltip" data-bs-title="time of day"}.

First, we convert the [doy]{.tool data-bs-toggle="tooltip" data-bs-title="day of year"} chosen below from days to years and then pass it to our [sda]{.tool data-bs-toggle="tooltip" data-bs-title="solar declination angle"} model. Then, we divide the [m&phi;]{.tool data-bs-toggle="tooltip" data-bs-title="millimeridians"} latitude selected below by 1000 to get the [&phi;]{.tool data-bs-toggle="tooltip" data-bs-title="meridian"} angle.  we can obtain the sunset and sunrise [tods]{.tool data-bs-toggle="tooltip" data-bs-title="times of day"}, which is the daytime duration, to obtain the difference between the sunset and solar noon [tods]{.tool data-bs-toggle="tooltip" data-bs-title="times of day"}, which is also the difference between the solar noon and sunrise [tods]{.tool data-bs-toggle="tooltip" data-bs-title="times of day"}, so that we can calculate sunrise and .
...
and then into an [sda]{.tool data-bs-toggle="tooltip" data-bs-title="solar declination angle"}, If we divide the result of the [sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"} equation by $\pi$, we get the daytime duration:

The in the [sha]{.tool data-bs-toggle="tooltip" data-bs-title="solar hour angle"} equation returns radians. To convert radians to turns, we divide by $\tau$. We then either  The difference between the sunset and sunrise [tods]{.tool data-bs-toggle="tooltip" data-bs-title="times of day"} is the daytime duration: ${selectedSunset} - ${selectedSunrise} = ${selectedDuration}.

$$\text{sunrise} = \text{noon} - \text{sha} \div \tau$$

The [m&lambda;]{.tool data-bs-toggle="tooltip" data-bs-title="milliparallels"} range input gives us solar noon. 

The line chart beneath the [doy]{.tool data-bs-toggle="tooltip" data-bs-title="day of year"}, [m&phi;]{.tool data-bs-toggle="tooltip" data-bs-title="millimeridians"} and [m&lambda;]{.tool data-bs-toggle="tooltip" data-bs-title="milliparallels"} range inputs visualizes sunrise, solar noon, and sunset for every [doy]{.tool data-bs-toggle="tooltip" data-bs-title="day of year"}, similar to the daylight area chart beneath the map🗺above. The main difference between the charts is that the area chart shows local [tods]{.tool data-bs-toggle="tooltip" data-bs-title="times of day"} and thus is independent of longitude, whereas the line chart includes [tzos]{.tool data-bs-toggle="tooltip" data-bs-title="time zone offsets"} and therefore changes with longitude.
...
$$\text{duration} = \text{sunset} - \text{sunrise}$$

The sunrise and sunset times in the equations above depend on the time zone of solar noon. To select a time zone  may be strange to consider The range🎚️inputs below set the [geographic coordinates](https://en.wikipedia.org/wiki/Geographic_coordinate_system#:~:text=positions%20directly%20on%20Earth%20as%20latitude%20and%20longitude) that are used for the equations above
...
selectedSunset = parseFloat((selectedSolarNoon + noonDiff).toFixed(5))
...
populationByHemisphere = FileAttachment("../../asset/populationByHemisphere.json").json();
...
sortedPopByHemi = populationByHemisphere.sort(
