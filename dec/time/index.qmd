...
//| echo: false
//| label: distmap
viewof location = worldMapCoordinates([[turn2long(table.rows[1].cells[0].childNodes[0].innerText), turn2degr(table.rows[1].cells[1].childNodes[0].innerText)], projection], [width, height * mapsize / 100])
```
:::
...
function worldMapCoordinates(config = {}, dimensions) {
  ...
  function draw() {
    ...
    table.rows[1].cells[0].innerHTML = createCellDiv(long2turn(lon), 10)
    table.rows[1].cells[1].innerHTML = createCellDiv(lati2turn(lat), 2.5)
  }
  let drag = d3.drag()
    .on("drag", (event) => {
  ...
  table.onkeyup = function(ev) {
   if ([
    // https://www.freecodecamp.org/news/javascript-keycode-list-keypress-event-key-codes#heading-a-full-list-of-key-event-values
      ...
      lon = turn2long(liveTable[0].Milliparallel);
      lat = turn2degr(liveTable[0].Millimeridian);
      table.rows[1].cells[0].innerHTML = createCellDiv(long2turn(lon), 10)
      table.rows[1].cells[1].innerHTML = createCellDiv(lati2turn(lat), 2.5)
      draw();
      canvas.dispatchEvent(new CustomEvent("input", { bubbles: true }));
