...
```{ojs}
//| echo: false
viewof location = worldMapCoordinates([162, 0], [960, 580])
```
:::
...
globe = (root, { width, location, date, hour }) => {
  ...
  root
    ...
    .attr("text-anchor", "middle")
    .attr("font-size", fontSize * (width < 600 ? 1 : 1.2) + 3)
...
function worldMapCoordinates(config = {}, dimensions) {
  ...
  canvas.style.margin = `-50px 10px -26px 10px`;
  const projection = d3
    .geoEquirectangular()
    .precision(0.1)
  ...
  const form = input({
    ...
    title,
    description,
    display: v => (width > 400) ? html`<div style="width: ${width}px; white-space: nowrap; color: #444; text-align: center; font: 18px monospace; position: relative; top: -3.1em;  margin-bottom: -.4em;">
