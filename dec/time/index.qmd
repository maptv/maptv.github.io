...
My website provides many examples of the [Quarto](https://quarto.org) publishing and the [Dec](/dec) measurement systems in action. I leverage Quarto support for the [Observable](https://observablehq.com/) data analysis and visualization system to create animated and interactive graphics like the clocks🕓and bar📊charts below, which show the Dec time since the start, [+\${decTime}]{.cyan}, and until the end, [+\${decTimeN}]{.pink}, of today.
...
Only the first digit of the Dec time, the deciday, varies across time zones, because the 10 Dec time zones, numbered 0 through 9 on the map🗺️, are each 1 deciday wide. Subtracting the time zone from the time yields the time in Zone 0.
