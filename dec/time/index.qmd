...
app = {
  ...
  const plot = svg.append("g")
    .attr("id", "plot")
    .attr("transform", `translate(${margin.left + 0}, ${width > 500 ? -72 : 5})`);
  svg.append("g")
...
function getSha(latitude, solarDeclinationAngle, solarZenithAngle = 0.252314) {
  ...
  return ratio >= 1 ? Math.PI : ratio <= -1 ? 0 : Math.acos(ratio)
}
...
sha = getSha(latInput / 1000, getSda(doyInput))
daytimeDuration = sha / Math.PI
...
daylightPlot = (
  ...
  legend
    .append("rect")
    .attr("x", width < 300 ? 63 : width < 500 ? 62 : width < 600 ? 61 : width < 700 ? 51 : width < 800 ? 41 : 30)
    .attr("y", width < 450 ? 8 : width < 850 ? 1 : -5)
    .attr("rx", 5)
  ...
  legend
    .append("text")
    .attr("x", width < 400 ? 82 : width < 500 ? 85 : width < 600 ? 77 : width < 700 ? 67 : width < 800 ? 57 : width < 900 ? 47 : 40)
  ...
  legend
    .append("rect")
    .attr("x", width < 400 ? -10 : width < 500 ? -20 : width < 600 ? -40 : width < 700 ? -60 : width < 800 ? -80 : -109)
    .attr("y", width < 450 ? 8 : width < 850 ? 1 : -5)
    .attr("rx", 5)
    .attr("width", fontSize * (width < 300 ? .95 : width < 400 ? 1 : width < 500 ? 1.05 : width < 600 ? 1.15 : width < 700 ? 1.2 : 1.25) + width / 100)
    .attr("height", fontSize * (width < 300 ? .95 : width < 400 ? 1 : width < 500 ? 1.05 : width < 600 ? 1.15 : width < 700 ? 1.2 : 1.25) + width / 100)
  ...
  legend
    .append("text")
    .attr("x", width < 400 ? 11 : width < 500 ? 4 : width < 600 ? -12 : -48)
    .attr("y", width < 300 ? 22 : width < 400 ? 23 : width < 450 ? 24 : width < 500 ? 18 : width < 600 ? 9 : width < 700 ? -1 : -3)
    .attr("font-size", fontSize * (width < 300 ? .95 : width < 400 ? 1 : width < 500 ? 1.05 : width < 600 ? 1.15 : width < 700 ? 1.2 : 1.25) + width / 100)
    .attr("font-family", "sans-serif")
    .text("Night");
...
function worldMapCoordinates(config = {}, dimensions) {
  ...
  const form = input({
    ...
    title,
    description,
    display: v => (width > 300) ? html`<div style="width: ${width}px; white-space: nowrap; color: window.darkmode ? #fff : #000; text-align: center; font: ${width > 1000 ? width / 50 : width / 40}px monospace; position: relative; top: ${-16 - width / 50}px;  margin-bottom: -.4em;">
...
sunclock = {
  ...
  const { minutes: _minutes, seconds } = minutesAndSeconds;
  //let _minutes = 0;
  const minutes = changed ? travel : _minutes;
  const minutesDegrees = (minutes / 1440) * 360;
  const svg = d3
    .create("svg")
    .attr("id", "btmclock")
  ...
  const sun = face
    .append("circle")
    .attr("r", 15)
    .attr("cx", 125 * -sin(minutesDegrees))
    .attr("cy", 125 * cos(minutesDegrees))
    .style("fill", "white");
  ...
  const digitalClock = svg
    ...
    .attr(
      "transform",
      `translate(${width / 2 + 45 * sin(minutesDegrees)}, ${
        height / 2 + 45 * -cos(minutesDegrees)
      })`
    );
...
//    .attr("transform", (d) => `rotate(${180 + d * 6})`);
  const clockText = digitalClock
    ...
    .attr("font-size", "75px")
    .text(minutesTo12HourTime(minutes));
  const ticks = face
    .selectAll(".tick")
    .data(d3.range(0, 20))
    .enter()
...
viewof changed = Inputs.input(false)
viewof travel = {
  const input = Inputs.range([0, 1440], {
    label: "Time Travel",
    step: 1,
    value: new Date().getHours() * 60 + new Date().getMinutes()
  });
  input.oninput = (x) => {
    if (!changed) set(viewof changed, true);
  };
  return input;
}
viewof reset = Inputs.button([["Reset", () => set(viewof changed, false)]])
...
hour_angle = toDegrees(
  Math.acos(
    (sin(-0.833) - sin(latitude) * Math.sin(sun_declination)) /
      (cos(latitude) * Math.cos(sun_declination))
  )
)
sun_transit = 720 - 4 * longitude - equation_of_time + timezone * 60
...
getDateFromSun = (v) => {
  ...
  d.setHours(v / 60);
  d.setMinutes(v % 60);
  d.setSeconds((v / 60) % 60);
  return d.toLocaleString();
}
sin = (x) => Math.sin(toRadians(x))
cos = (x) => Math.cos(toRadians(x))
m_ = {
  let lastMinutes = undefined;
  let { minutes } = minutesAndSeconds;
...
timezone = getTimezoneFromLatLng(latitude, longitude)
import { timezones } from "@mbostock/time-zones"
...
.clocks {
  margin-top: ${width > 500 ? -62 : width > 400 ? 80 : 0}px;
}
</style>
`
```
