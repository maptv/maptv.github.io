...
Dec dates only include the first two terms from the [left-hand side](https://en.wikipedia.org/wiki/Sides_of_an_equation#:~:text=the%20expression%20on%20the%20left%20of%20the%20%22%3D%22%20is%20the%20left%20side%20of%20the%20equation) of the Dec date equation: $\lfloor\colorbox{yellow}{y}\rfloor$ and $\colorbox{cyan}{d}$. The right-hand side of the equation, $\colorbox{yellow}{y}$, can also be used as a date. In contrast, $\colorbox{orange}{n}$ does not help us to identify a specific date, but is required to calculate $\colorbox{yellow}{y}$ via the Dec date equation above. To find $\colorbox{orange}{n}$, we plug $\lfloor\colorbox{yellow}{y}\rfloor$ into the Dec year length equation below:
...
The `leap` function below can be used to obtain $\colorbox{orange}{n}$, which in turn can be used to turn $\lfloor\colorbox{yellow}{y}\rfloor$ and $\colorbox{cyan}{d}$ into $\colorbox{yellow}{y}$. The `leap` function is not used to create Dec dates, but can be used to turn existing `year+day` Dec dates into `year-day` Dec dates according to an expanded version of the yote equation: $\colorbox{yellow}{y}{=}\lfloor\colorbox{yellow}{y}\rfloor{+}\colorbox{cyan}{d}{\div}\colorbox{orange}{n}{=}\lfloor\colorbox{yellow}{y}\rfloor{+}1-(\colorbox{orange}{n}-\colorbox{cyan}{d}){\div}\colorbox{orange}{n}$.

The current `year-day` date, \${styledNextYear}-\${styledDecoTminus}, tells us that there are \${Tminus} days left in the year. In addition to revealing how many days remain in the year, `year-day` dates can also be useful for [calendrical calculations](https://en.wikipedia.org/wiki/Calendrical_calculation#:~:text=calculation%20concerning%20calendar%20dates). The `date` function below demonstrates that the Dec dates `1600+146097` and `2400-146097` are both equivalent to `2000+000`.

In essence, Dec treats each year $\lfloor\colorbox{yellow}{y}\rfloor$ like an [array](https://en.wikipedia.org/wiki/Array_(data_structure)#Element_identifier_and_addressing_formulas:~:text=a%20data%20structure%20consisting%20of%20a%20collection%20of%20elements%20(values%20or%20variables)%2C%20of%20same%20memory%20size%2C%20each%20identified%20by%20at%20least%20one%20array%20index) and each day of the year $\colorbox{cyan}{d}$ as an array [index](https://en.wikipedia.org/wiki/Array_(data_structure)#Element_identifier_and_addressing_formulas:~:text=individual%20objects%20are%20selected%20by%20an%20index) that can be used to identify array elements or combine them via [slicing](https://en.wikipedia.org/wiki/Array_slicing#:~:text=an%20operation%20that%20extracts%20a%20subset%20of%20elements%20from%20an%20array). Dec slices have their own equation.
...
styledNextYear = setStyle(parseInt(decoYear) + 1, d3.schemePaired[10])
