...
The distinction between positive and negative [doys]{.cyan .under .tool data-bs-toggle="tooltip" data-bs-title="days of year"} can also be explained in terms of computer programming. If we think of years as [arrays](https://en.wikipedia.org/wiki/Array_(data_structure)#Element_identifier_and_addressing_formulas:~:text=a%20data%20structure%20consisting%20of%20a%20collection%20of%20elements%20(values%20or%20variables)%2C%20of%20same%20memory%20size%2C%20each%20identified%20by%20at%20least%20one%20array%20index), [n]{.orange} is the number of elements in the array, and [doys]{.cyan .under .tool data-bs-toggle="tooltip" data-bs-title="days of year"} are like [positive](https://en.wikipedia.org/wiki/Zero-based_numbering#:~:text=a%20way%20of%20numbering%20in%20which%20the%20initial%20element%20of%20a%20sequence%20is%20assigned%20the%20index%C2%A00) or [negative index](https://en.wikipedia.org/wiki/Array_slicing#:~:text=specify%20an%20offset%20from%20the%20end%20of%20the%20array) array [indexes](https://en.wikipedia.org/wiki/Array_(data_structure)#Element_identifier_and_addressing_formulas:~:text=individual%20objects%20are%20selected%20by%20an%20index) that can be used to identify array elements or combine them into groups via [slicing](https://en.wikipedia.org/wiki/Array_slicing#:~:text=an%20operation%20that%20extracts%20a%20subset%20of%20elements%20from%20an%20array). 

In Dec, a [doy]{.tool data-bs-toggle="tooltip" data-bs-title="day of year"} by itself is a floating🛟date and a [yoe]{.tool data-bs-toggle="tooltip" data-bs-title="year of era"}+[doy]{.tool data-bs-toggle="tooltip" data-bs-title="day of year"} date is a anchored⚓️date. Unlike anchored⚓️dates, floating🛟dates do not include a year and thus can apply to any year. Anchored⚓️dates are [unsimplified](https://en.wikipedia.org/wiki/Simplification#:~:text=the%20process%20of%20replacing%20a%20mathematical%20expression%20by%20an%20equivalent%20one%2C%20that%20is%20simpler) math expressions. Instead of simplifying the expression, we can expand it to display additional information.

For example, we can expand the minuend \${decDoty} to get the subtrahend 299 and the difference \${xmasDiff}, which tell us that \${Math.abs(xmasDiff)} days \${xmasDiffSince} [Day 299]{.tool data-bs-toggle="tooltip" data-bs-title="December 25"}🎄of this [year]{.yellow}. If we were preparing for a rocket🚀launch, the minuend would be the current time, the subtrahend would be the planned launch time, and the difference would be the ["T-minus" countdown](https://en.wikipedia.org/wiki/Countdown#:~:text=backward%20counting%20to%20indicate%20the%20time%20remaining%20before%20an%20event).


To see its minuend, subtrahend, and difference at the same time, we could rewrite the expanded date from the example above⬆️, \${decYear}+299[\${xmasDiffSign}]{.mono}\${Math.abs(xmasDiff)}, as a Dec span🌈: \${decYear}+\${decDoty}=\${decYear}+299[\${xmasDiffSign}]{.mono}\${Math.abs(xmasDiff)}. Unlike expanded dates, Dec spans🌈represent time intervals instead of individual dates and are structured like an equation as opposed to a math expression that can be simplified to fractional year.
