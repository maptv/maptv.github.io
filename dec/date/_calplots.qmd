...
decPlot = !turnInput ? Plot.plot({
  ...
  marks: [
    Plot.cell(dates, {
      ...
      x: (d, i) => i % 10,
      fill: (d, i) => i === dotyInput && leapInput && !coloInput ? leap10[i] : i === dotyInput && !coloInput ? comm10[i] : i === dotyInput ? "selected" : coloInput ? months[(new Date(d.getTime() - nOffInput * 86400000)).getUTCMonth()] : leapInput ? leap08[i] : comm08[i],
    ...
    Plot.text(dates, {
      y: (d, i) => Math.floor(i / 10),
      x: (d, i) => i % 10,
      fill: (d, i) => !coloInput && i > 199 ? "white" : "black",
      text: (d, i) => String(i),
      monospace: true,
      fontSize: "16px"})
    ]
