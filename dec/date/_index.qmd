...
leaps = d3.range(366).map(x => d3.hsl(piecewiseColor(x / 366)))
comms = d3.range(365).map(x => d3.hsl(piecewiseColor(x / 365)))
leaphues = leaps.map(x => x.h)
commhues = comms.map(x => x.h)
leap10 = leaps.map(x => x.formatHex())
comm10 = comms.map(x => x.formatHex())
leap08 = leaphues.map(x => d3.hsl(x, .8, .6).formatHex())
comm08 = commhues.map(x => d3.hsl(x, .8, .6).formatHex())
```
