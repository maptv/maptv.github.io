...
b32pl = "AĄBCĆDEĘFGHIJKLŁMNŃOÓPRSŚTUWYZŹŻ";
...
function reverseBits() {
  const [top, btm] = reverseWindows(ranintop, raninbtm, multiplier, 5)
  set(viewof ranintop, top)
  set(viewof raninbtm, btm)
}
function stepPattern() {
  const [top, btm] = stepBitwise(10, multiplier)
  set(viewof ranintop, top)
  set(viewof raninbtm, btm)
}
function reverseWindows(top, btm, win = 1, halflen = 5) {
  const combined = (top << halflen | btm).toString(2).slice(-10).padStart(10, "0");
  win = win === 1 ? combined.length : win
  let out = "";
  for (let i = 0; i < combined.length; i += win) {
    const window = combined.slice(i, i + win);
    out += window.split("").reverse().join("");
  }
  out = parseInt(out, 2)
  return [out >>> halflen, out & (2**halflen - 1)];
}
function stepBitwise(length, step) {
  let out = "";
