...
In contrast, [Q2], [G3], and [N3] make it possible to skip local rendering in favor of relying on [GitHub Actions](https://quarto.org/docs/publishing/quarto-pub.html#github-action) to handle all of the necessary steps. Before trying to use GitHub Actions or any [other continuous integration systems](https://quarto.org/docs/publishing/ci.html) in your publishing workflow, I suggest getting used to working with `quarto` `publish`.

In @sec-ghp, I will walk through the setup of both [G2] and [G3] to provide the option of rendering locally by running `quarto` `publish` or rendering remotely with GitHub Actions by pushing to the `main` branch. [G3] can easily be combined [Q2] and [N3], but I decided to stick with [N2].

Along the way, I will share many practical tips and general advice that you can apply to any project.
...
To set up [G3] alongside [G2], all we have to do is create a `.github/workflows` directory and add a `publish.yml` file, like the one displayed in @exm-pub. To create the required directory and obtain the `publish.yml` I use for my site, you can use the shell shown in @exm-curl.
...
The `publish.yml` file in @exm-pub installs R, Python, and the packages in the `renv.lock` and `requirements.txt` files.
