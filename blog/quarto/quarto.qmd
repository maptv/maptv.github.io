...
With `quarto` `publish`, you can render your site and then deploy it with a single [shell](https://en.wikipedia.org/wiki/Unix_shell#:~:text=A%20Unix%20shell%20is%20a%20command%2Dline%20interpreter%20or%20shell%20that%20provides%20a%20command%20line%20user%20interface%20for%20Unix%2Dlike%20operating%20systems.) command. Unfortunately, `quarto` `publish` does not work for GitLab Pages and has additional requirements for GitHub Pages.

To set up GitHub Pages site to work with `quarto` `publish`, your [source](https://quarto.org/docs/publishing/github-pages.html#source-branch) [branch](https://en.wikipedia.org/wiki/Branching_(version_control)#:~:text=Branching%2C%20in%20version%20control%20and%20software%20configuration%20management%2C%20is%20the%20duplication%20of%20an%20object%20under%20version%20control) must be set to `gh-pages`. Importantly, you cannot have the `gh-pages` branch checked out when running `quarto` `publish` or else you will get an error: `fatal: 'gh-pages' is already checked out at...`.

No additional setup is required to run `quarto` `publish` with [Quarto Pubs](https://quarto.org/docs/publishing/quarto-pub.html) or [Netlify](https://quarto.org/docs/publishing/netlify.html), but going through the extra setup for [GitHub Pages](https://quarto.org/docs/publishing/github-pages.html) can essentially replace the Git workflow shown in @fig-git with the `quarto` `publish` command.
