...
In @sec-pages, I will walk through the setup of both [G2] and [G3] to provide the option of rendering locally by running `quarto` `publish` or rendering remotely with GitHub Actions by pushing to the `main` branch. Along the way, I will share many practical tips and general advice that you can apply to any project.

## GitHub and GitLab Pages setup {#sec-pages}
...
To also automatically update my site on Quarto Pub, I added a `quarto-pub.yml` file to my `.github/workflows`. Unlike `gh-pages.yml`, `quarto-pub.yml` runs upon completion of the `pages-build-deployment` workflow, does not render, and only the `gh-pages` per [Q2]. With this change, my GitHub Actions workflow is capable of publishing my site on all four web hosts every time I push to `main`!
