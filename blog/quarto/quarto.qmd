...
To also automatically update my site on Quarto Pub, I added a `quarto-pub.yml` file to my `.github/workflows`. Unlike `gh-pages.yml`, `quarto-pub.yml`  upon a push to the `gh-pages` per [Q2]. With this change, my GitHub Actions workflow is capable of publishing my site on all four web hosts every time I push to `main`!
