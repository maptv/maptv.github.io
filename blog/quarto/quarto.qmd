...
I added a "Push to GitLab" step to my `publish.yml` to make GitHub Actions push my remote `gh-pages` to GitLab so that my site is kept in sync on both GitHub and GitLab Pages. This required manually creating a token on GitLab and adding it to GitHub, which I accomplished using the GitHub CLI as shown in @exm-secret.
...
To also automatically update my site on Quarto Pub, I added a "Publish to Quarto Pub" step to `publish.yml` file as per [Q2]. With this change, my GitHub Actions workflow is capable of publishing my site on all four web hosts every time I push to `main`!
