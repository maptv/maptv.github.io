...
With the settings described above, running `quarto` `publish` `gh-pages` updates my site on [GitHub](https://pages.github.com), [GitLab](https://docs.gitlab.com/ee/user/project/pages), and [Netlify](https://www.netlify.com). To also deploy to my site to [Quarto Pub](https://quarto.pub), I need to run `quarto` `publish` `quarto-pub`.

I can use a shell [alias](https://en.wikipedia.org/wiki/Alias_(command)#:~:text=In%20computing%2C%20alias%20is%20a%20command%20in%20various%20command%2Dline%20interpreters%20(shells)%2C%20which%20enables%20a%20replacement%20of%20a%20word%20by%20another%20string.) to combine both steps 

::: {#exm-qp}
```{.sh}
alias qp='(
quarto publish --no-prompt --no-browser gh-pages;
quarto publish --no-prompt --no-render quarto-pub;
)'
```
:::

This easier than instead of running [`quarto` `render`]() and then the three commands in the shown in @fig-git. 
To set up GitHub Pages site to work with `quarto` `publish`, your [source](https://quarto.org/docs/publishing/github-pages.html#source-branch) [branch](https://en.wikipedia.org/wiki/Branching_(version_control)#:~:text=Branching%2C%20in%20version%20control%20and%20software%20configuration%20management%2C%20is%20the%20duplication%20of%20an%20object%20under%20version%20control) must be set to `gh-pages`. Importantly, you cannot be on the `gh-pages` branch when running `quarto` `publish` or else you will get an error: `fatal: 'gh-pages' is already checked out at...`.

No additional setup is required to run `quarto` `publish` with [Quarto Pubs](https://quarto.org/docs/publishing/quarto-pub.html) or [Netlify](https://quarto.org/docs/publishing/netlify.html), but going through the extra setup for [GitHub Pages](https://quarto.org/docs/publishing/github-pages.html) can essentially replace the Git workflow shown in @fig-git with the `quarto` `publish` command. This way, you get the benefits of the Git version control system and GitHub without giving up the ease bbb
