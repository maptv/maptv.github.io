...
[G3] setup only requires that we create a `.github/workflows` directory and add a `publish.yml` file (not to be confused with `_publish.yml`) to that directory. @exm-curl contains the `publish.yml` file I use for my own site and the shell code that can used to obtain this file.
...
After pushing the `.github/workflows/publish.yml` file, you can visit the Actions tab in your remote repo on GitHub to check the progress of the deployment of your site. If your site did not build successfully, you can go through the logs to try to diagnose the problem.

I configured my `publish.yml` to have GitHub Actions push my remote `gh-pages` to GitLab so that my site is kept in sync on both GitHub and GitLab Pages. This required manually creating a token on GitLab and adding it to GitHub, which I accomplished using the GitHub CLI as shown in @exm-secret.
...
Instead of using GitHub Actions, I could have expanded my `.gitlab-ci.yml` file to have [GitLab CI/CD](https://docs.gitlab.com/ee/ci) build my site. I decided not to go down this route because the Quarto dev team has many [GitHub Actions workflows](https://github.com/quarto-dev/quarto-actions) available but no official support yet for GitLab CI/CD.

To summarize, I have two options for publishing my site: 1) running `quarto` `publish` and 2) running `git push` ([G2]) + [N2]
