...
@tbl-pub uses a `Code` derived from numbered lists in the Quarto publishing documentation to identify different publishing strategies that I discuss in this blog post. Other than the `Host`, the methods are distinguished by local or remote rendering and the whether the site is deployed with the [`quarto` `publish`](https://quarto.org/docs/output-formats/html-publishing.html#publish-command) or  [`git` `push`](https://git-scm.com/docs/git-push) [shell](https://en.wikipedia.org/wiki/Unix_shell#:~:text=A%20Unix%20shell%20is%20a%20command%2Dline%20interpreter%20or%20shell%20that%20provides%20a%20command%20line%20user%20interface%20for%20Unix%2Dlike%20operating%20systems.) command.
...
The main method to deploy Quarto sites is the `quarto` `publish` command, which can [render](https://quarto.org/docs/websites/index.html#website-render) and deploy your site in one fell swoop and then open it a browser so you can view the latest version. The [Q1], [N1], and [G2] publishing methods in @tbl-pub all use `quarto` `publish.`
...
All of the approaches that use [GitHub Pages](https://pages.github.com) as a `Host` can be adapted to work for [GitLab Pages](https://docs.gitlab.com/ee/user/project/pages). 
