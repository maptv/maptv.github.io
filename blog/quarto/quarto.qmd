...
With the setup described in @sec-act, I can publish my site on all four web hosts every time I run `quarto` `publish` `gh-pages` ([G2]) or `git` `push` ([G3])! @fig-meth summarizes all of the steps that occur during my Quarto publishing workflow.
...
If typically only use [G3], but if I want to switch to [G2], I have to run `git` `fetch` `origin` `gh-pages:gh-pages` to update my local `gh-pages` branch with any commits created remotely. Otherwise, I would have conflicting local and remote histories in my `gh-pages` branch and might have to fix a merge conflict.
