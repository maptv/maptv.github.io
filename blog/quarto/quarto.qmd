...
@tbl-pub uses a `Code` derived from numbered lists in the Quarto publishing documentation to identify different publishing strategies that I discuss in this blog post. The publishing methods in @tbl-pub each target a particular web `Host`, `Render` content locally or on a remote server, and deploy sites either the [`quarto` `publish`](https://quarto.org/docs/output-formats/html-publishing.html#publish-command) or  the [`git` `push`](https://git-scm.com/docs/git-push) [shell](https://en.wikipedia.org/wiki/Unix_shell#:~:text=A%20Unix%20shell%20is%20a%20command%2Dline%20interpreter%20or%20shell%20that%20provides%20a%20command%20line%20user%20interface%20for%20Unix%2Dlike%20operating%20systems.) `Command.`
