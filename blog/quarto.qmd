...
The first topic I will delve into is creating and publishing a website with Quarto. To get started, follow the Quarto documentation on [creating a blog](https://quarto.org/docs/websites/website-blog.html). As soon as you create the starter template, you can set it up to be hosted by one of the many available publishing services.

To demonstrate some of the many free website hosting options, I published four identical blogs: <https://maptv.quarto.pub>, <https://maptv.netlify.app>, <https://maptv.github.io>, and <https://maptv.gitlab.io>. If want to host your site on [Quarto Pub](https://quartopub.com), [Netlify](https://www.netlify.com), or [GitHub Pages](https://pages.github.com), the Quarto [publishing documentation](https://quarto.org/docs/publishing/#getting-started) is all you need.

If you prefer publishing to [GitLab Pages](https://docs.gitlab.com/ee/user/project/pages/), you will need to have a [GitLab account](https://gitlab.com/users/sign_up) and a [Git](https://en.wikipedia.org/wiki/Git#:~:text=Git%20(/%C9%A1%C9%AAt/)%5B8%5D%20is%20a%20distributed%20version%20control%20system%5B9%5D%20that%20tracks%20changes%20in%20any%20set%20of%20computer%20files%2C%20usually%20used%20for%20coordinating%20work%20among%20programmers%20who%20are%20collaboratively%20developing%20source%20code%20during%20software%20development.) [repository](https://en.wikipedia.org/wiki/Repository_(version_control)#:~:text=In%20version%20control%20systems%2C%20a%20repository%20is%20a%20data%20structure%20that%20stores%20metadata%20for%20a%20set%20of%20files%20or%20directory%20structure.) containing a `.gitlab-ci.yml` file like the one below. After pushing your rendered content to a GitLab repository, uncheck **Use unique domain** under `Deploy > Pages` in the [GitLab](https://gitlab.com) sidebar to have a simpler [URL](https://en.wikipedia.org/wiki/URL#:~:text=A%20Uniform%20Resource%20Locator%20(URL)%2C%20colloquially%20known%20as%20an%20address%20on%20the%20Web).
...
If you are interested in learning about Git, take a look at the ["GitHub for supporting, reusing, contributing, and failing safely"](https://openscapes.org/blog/2022-05-27-github-illustrated-series/) post by [Allison Horst](https://allisonhorst.com) and [Julie Lowndes](https://jules32.github.io) on the [Openscapes](https://openscapes.org/) [blog](https://openscapes.org/blog). If not, you may prefer to host your site on [Quarto Pubs](https://quarto.org/docs/publishing/quarto-pub.html) or [Netlify](https://quarto.org/docs/publishing/netlify.html).

The `quarto publish` command is an easy way to update sites hosted on [Quarto Pub](https://quarto.org/docs/publishing/netlify.html#publish-command), [Netlify](https://quarto.org/docs/publishing/quarto-pub.html#publish-command), and [GitHub Pages](https://quarto.org/docs/publishing/github-pages.html#publish-command), but it does not work for GitLab Pages. Luckily, GitHub and GitLab Pages sites can be updated using the `git push` command.

Git is very powerful
I set up my local git [repository](https://en.wikipedia.org/wiki/Repository_(version_control)#:~:text=In%20version%20control%20systems%2C%20a%20repository%20is%20a%20data%20structure%20that%20stores%20metadata%20for%20a%20set%20of%20files%20or%20directory%20structure.) with a second remote git repository [url](https://en.wikipedia.org/wiki/URL#:~:text=A%20Uniform%20Resource%20Locator%20(URL)%2C%20colloquially%20known%20as%20an%20address%20on%20the%20Web) called called `lab` by running `git remote add lab https://gitlab.com/maptv/maptv.gitlab.io`. I then add the `lab` url as a second `origin` remote url by running `git remote set-url origin $(git remote get-url lab) --add`.
