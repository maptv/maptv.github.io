...
The first topic I will delve into is creating and publishing a website with Quarto and optionally [Git](https://en.wikipedia.org/wiki/Git#:~:text=Git%20(/%C9%A1%C9%AAt/)%5B8%5D%20is%20a%20distributed%20version%20control%20system%5B9%5D%20that%20tracks%20changes%20in%20any%20set%20of%20computer%20files%2C%20usually%20used%20for%20coordinating%20work%20among%20programmers%20who%20are%20collaboratively%20developing%20source%20code%20during%20software%20development.). Git is a version control and collaboration system used by 93% of software developers worldwide, according to [survey results](https://stackoverflow.blog/2023/01/09/beyond-git-the-other-version-control-systems-developers-use/#:~:text=Our%20developer%20survey%20found%2093%25%20of%20developers%20use%20Git.) published by [StackOverflow](https://stackoverflow.com/tour) on `2022+314`.

If you are interested in Git, take a look at the ["GitHub for supporting, reusing, contributing, and failing safely"](https://openscapes.org/blog/2022-05-27-github-illustrated-series/) post by [Allison Horst](https://allisonhorst.com) and [Julie Lowndes](https://jules32.github.io) on the [Openscapes](https://openscapes.org/) [blog](https://openscapes.org/blog). In this blog post, I will briefly Regardless of your interest (or lack thereof) If not, you can skip any instructions below marked with an [orange]{.orange} "Git" [callout](https://quarto.org/docs/authoring/callouts.html#callout-types) and still publish your website by following instructions marked with [green]{.green} "No Git" callout.

To demonstrate various free website hosting options, I published my blog on [GitHub](https://maptv.github.io), [GitLab](https://maptv.gitlab.io), [Quarto Pub](https://maptv.quarto.pub), and [Netlify](https://maptv.netlify.app).

I first followed The [Quarto documentation](https://quarto.org/docs) covers [publishing](https://quarto.org/docs/publishing) to [Quarto Pubs](https://quarto.org/docs/publishing/quarto-pub), [Netlify](https://quarto.org/docs/publishing/netlify), and [GitHub Pages](https://quarto.org/docs/publishing/github-pages), but not [GitLab Pages](https://docs.gitlab.com/ee/user/project/pages/).

To set up all of the above, 

# Publishing without Git

https://en.wikipedia.org/wiki/URL#:~:text=A%20Uniform%20Resource%20Locator%20(URL)%2C%20colloquially%20known%20as%20an%20address%20on%20the%20Web
I followed the Quarto documentation to [create a blog](https://quarto.org/docs/websites/website-blog.html) and [publish it](https://quarto.org/docs/publishing/) to [Quarto Pub](https://quarto.org/docs/publishing/quarto-pub.html), and [Netlify](https://quarto.org/docs/publishing/netlify.html). All three of these are awesome free hosting options, and you only need one, but I also wanted my site to be hosted on [GitLab Pages](https://docs.gitlab.com/ee/user/project/pages/), so I added the `.gitlab-ci.yml` file shown below and unchecked **Use unique domain** under `Deploy > Pages` in the [GitLab](https://gitlab.com) sidebar. 
...
Next, I set up my local git [repository](https://en.wikipedia.org/wiki/Repository_(version_control)#:~:text=In%20version%20control%20systems%2C%20a%20repository%20is%20a%20data%20structure%20that%20stores%20metadata%20for%20a%20set%20of%20files%20or%20directory%20structure.) with a second remote git repository [url](https://en.wikipedia.org/wiki/URL#:~:text=A%20Uniform%20Resource%20Locator%20(URL)%2C%20colloquially%20known%20as%20an%20address%20on%20the%20Web) called called `lab` by running `git remote add lab https://gitlab.com/maptv/maptv.gitlab.io`. I then add the `lab` url as a second `origin` remote url by running `git remote set-url origin $(git remote get-url lab) --add`.

I update all four sites at the same time with a shell [alias](https://en.wikipedia.org/wiki/Alias_(command)#:~:text=In%20computing%2C%20alias%20is%20a%20command%20in%20various%20command%2Dline%20interpreters%20(shells)%2C%20which%20enables%20a%20replacement%20of%20a%20word%20by%20another%20string.): `alias qp="func() { quarto publish --no-prompt --no-browser --no-render quarto-pub; quarto publish --no-prompt --no-browser --no-render netlify; git add --all && git commit --message \"$(echo '${*:-Changed files: $(echo $(git status --porcelain | grep -v "?" | cut -c4- | tr "\n" " "))}')\" && git push; }; func"`. This alias allows me to provide a [git commit](https://en.wikipedia.org/wiki/Commit_(version_control)#:~:text=In%20version%20control%20systems%2C%20a%20commit%20is%20an%20operation%20which%20sends%20the%20latest%20changes%20of%20the%20source%20code%20to%20the%20repository%2C%20making%20these%20changes%20part%20of%20the%20head%20revision%20of%20the%20repository.) message that describes each update directly on the [command line](https://en.wikipedia.org/wiki/Command-line_interface#:~:text=A%20command%2Dline%20interface%20(CLI)%20is%20a%20means%20of%20interacting%20with%20a%20computer%20program%20by%20inputting%20lines%20of%20text%20called%20command%2Dlines.), e.g. `qp rewrite first blog post`.

::: {.callout-warning}
Un-escaped special characters in the commit message following my `qp` alias will be interpreted as shell command operators! If you don't know what this means, I advise against trying to use this alias! 
:::

Working with GitHub and GitLab, implies familiarity with the 

There are many [git graphical user interfaces (GUIs)](https://git-scm.com/downloads/guis), and I generally suggest starting with [GitHub Desktop](https://desktop.github.com/), but accessing the full power of git requires learning to use the command line. I interact with the command line using [Z shell](https://en.wikipedia.org/wiki/Z_shell#:~:text=The%20Z%20shell%20(Zsh)%20is%20a%20Unix%20shell%20that%20can%20be%20used%20as%20an%20interactive%20login%20shell%20and%20as%20a%20command%20interpreter%20for%20shell%20scripting.%20Zsh%20is%20an%20extended%20Bourne%20shell%20with%20many%20improvements%2C%20including%20some%20features%20of%20Bash%2C%20ksh%2C%20and%20tcsh.), the default interactive [shell](https://en.wikipedia.org/wiki/Unix_shell#:~:text=A%20Unix%20shell%20is%20a%20command%2Dline%20interpreter%20or%20shell%20that%20provides%20a%20command%20line%20user%20interface%20for%20Unix%2Dlike%20operating%20systems.) on [MacOS](https://en.wikipedia.org/wiki/MacOS#:~:text=macOS%20(/%CB%8Cm%C3%A6ko%CA%8A%CB%88%C9%9Bs/%3B%5B6%5D%20previously%20OS%C2%A0X%20and%20originally%20Mac%C2%A0OS%C2%A0X)%20is%20an%20operating%20system%20developed%20and%20marketed%20by%20Apple%20Inc.) and [Kali Linux](https://en.wikipedia.org/wiki/Kali_Linux#:~:text=Kali%20Linux%20is%20a%20Debian%2Dderived%20Linux%20distribution%20designed%20for%20digital%20forensics%20and%20penetration%20testing.), which is compatible with the ubiquitous [Bourne Again Shell (bash)](https://en.wikipedia.org/wiki/Bash_(Unix_shell)#:~:text=Bash%20is%20a%20Unix%20shell%20and%20command%20language), the default interactive shell on most [Linux](https://en.wikipedia.org/wiki/Linux#:~:text=Linux%20(/%CB%88l%C9%AAn%CA%8Aks/%20LIN%2Duuks)%5B11%5D%20is%20a%20family%20of%20open%2Dsource%20Unix%2Dlike%20operating%20systems) [distributions](https://en.wikipedia.org/wiki/Linux_distribution#:~:text=A%20Linux%20distribution%5Ba%5D%20(often%20abbreviated%20as%20distro)%20is%20an%20operating%20system%20made%20from%20a%20software%20collection%20that%20includes%20the%20Linux%20kernel%2C%20and%20often%20a%20package%20management%20system.).

The `git push` command updates my blog site [repository](https://en.wikipedia.org/wiki/Repository_(version_control)#:~:text=In%20version%20control%20systems%2C%20a%20repository%20is%20a%20data%20structure%20that%20stores%20metadata%20for%20a%20set%20of%20files%20or%20directory%20structure.) on both [GitHub](https://github.com) and [GitLab](https://gitlab.com) because 

publish in two different places?  then  instructions. and was 
show explain how  barriers to benefit from Quarto regarit is easy to combine code, the output of the code, and text describing both the code and the output. in the form of It's the [bee's🐝knees](https://en.wiktionary.org/wiki/the_bee%27s_knees#:~:text=Something%20or%20someone%20excellent%2C%20surpassingly%20wonderful%2C%20or%20cool), because it

I created my blog using the [Quarto](https://quarto.org/#:~:text=An%20open%2Dsource%20scientific%20and%20technical%20publishing%20system), which is the

I taught web development

inline styles, like those in Quarto HTML blocks have the highest priority.
...
Coming up next on my blog is [a post](observable.qmd) that introduces `Dec` with [Observable](https://observablehq.com/) graphics. Get ready for an animated and interactive data visualization extravaganza!
